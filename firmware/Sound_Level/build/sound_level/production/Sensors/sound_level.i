...
void SoundLevel_PrepareData(){
    ...
    float dBZ = 10*log10f(presAvgSquared/(20e-6 * 20e-6));

    if(dBZ > 106){
        dBZ = 106;
    }

    uint16_t dataToSend = (uint16_t) roundf(600 * dBZ);
...
void SoundLevel_GetSample(){
    ...
    float sampleToVoltage = (3.3 * sample)/(4095);
