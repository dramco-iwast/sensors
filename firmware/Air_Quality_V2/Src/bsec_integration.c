...
bsec_data bsec_value;

extern uint8_t generateIntReq;
extern Device_API_t sensorAPI;
extern uint8_t mDataReq;
extern uint32_t n_samples;
extern int64_t timeOfMeasurementDataPrint;
extern uint8_t DelayULPPStart;
extern uint8_t mMeasurementReq;
                    ...
                    bme680_com_fptr_t bus_read, sleep_fct sleep, state_load_fct state_load, config_load_fct config_load)
{
    ...
    /* Fixed I2C configuration, changed to SPI configuration */
    bme680_g.dev_id = 0; // changed, was BME680_I2C_ADDR_PRIMARY;
    bme680_g.intf = BME680_SPI_INTF;  // changed! Was BME680_I2C_INTF
    /* User configurable I2C configuration */
    bme680_g.write = bus_write;
    bme680_g.read = bus_read;
    bme680_g.delay_ms = sleep;
    bme680_g.amb_temp = 25; // added
    ...
    {
    	uart_buf_len = sprintf(uart_buf, "Error while initializing BME680 API. bme680_status = %d\r\n",ret.bme680_status);
    ...
    {
    	uart_buf_len = sprintf(uart_buf, "Error while initializing BSEC library. bme680_status = %d\r\n",ret.bme680_status);
    	HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
        return ret;
    }
    ...
    if (bsec_config_len != 0)
    {
        ...
        if (ret.bsec_status != BSEC_OK)
        {
        	uart_buf_len = sprintf(uart_buf, "Error while loading BSEC library configuration. bme680_status = %d\r\n",ret.bme680_status);
        	HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
            return ret;
        }
    }
    ...
    if (bsec_state_len != 0)
    {
        ...
        if (ret.bsec_status != BSEC_OK)
        {
        	uart_buf_len = sprintf(uart_buf, "Error while loading previous BSEC library state, start without loading previous state. bsec_status = %d\r\n",ret.bsec_status);
        	HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
            //return ret;
        }
    }
    ...
    if (ret.bsec_status != BSEC_OK)
    {
    	uart_buf_len = sprintf(uart_buf, "Error while setting library subscription information. bme680_status = %d\r\n",ret.bme680_status);
    	HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
        return ret;
    }
...
void bme680_bsec_trigger_measurement(bsec_bme_settings_t *sensor_settings, sleep_fct sleep)
{
    ...
    {
		HAL_GPIO_WritePin(INT_GPIO_Port, LED_Pin, GPIO_PIN_RESET); // to be used as trigger when a measurement starts (via LED)
        ...
        {
      	  uart_buf_len = sprintf(uart_buf, "bme680_set_sensor_settings issue, bme680_status = %d.\r\n",(int)bme680_status);
          HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
        }
        ...
        if (bme680_status != BME680_OK)
        {
      	  uart_buf_len = sprintf(uart_buf, "bme680_set_sensor_mode issue, bme680_status = %d.\r\n",(int)bme680_status);
          HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
        }
        /* Get the total measurement duration so as to sleep or wait till the measurement is complete */
        bme680_get_profile_dur(&meas_period, &bme680_g);
      	uart_buf_len = sprintf(uart_buf, "bme680 measurement duration= %d.\r\n",(int)meas_period);
        HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);

        /* Delay till the measurement is ready. Timestamp resolution in ms */
        sleep((uint32_t)meas_period);
    }
	HAL_GPIO_WritePin(INT_GPIO_Port, LED_Pin, GPIO_PIN_SET); // to switch of LED

    /* Call the API to get current operation mode of the sensor */
    bme680_status = bme680_get_sensor_mode(&bme680_g);
    if (bme680_status != BME680_OK)
    {
  	  uart_buf_len = sprintf(uart_buf, "bme680_get_sensor_mode issue, bme680_status = %d.\r\n",(int)bme680_status);
      HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
    }
    ...
    while (bme680_g.power_mode == BME680_FORCED_MODE)
    {
  	...
  	//uart_buf_len = sprintf(uart_buf, "bme680 is out of BME680_FORCED_MODE.\r\n");
    //HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
}
...
//        if (bme680_status == 2) // Warning, no new data. Wait for 300ms and try again.
//        {
//    	  uart_buf_len = sprintf(uart_buf, "bme680_get_sensor_data issue, try again.\r\n");
//          HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
//
//          HAL_Delay(300);
//          bme680_status = bme680_get_sensor_data(&data, &bme680_g);
//        }
        if (bme680_status != BME680_OK)
        {
      	  uart_buf_len = sprintf(uart_buf, "bme680_get_sensor_data issue, bme680_status = %d.\r\n",(int)bme680_status);
          HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
        }
        ...
        for (index = 0; index < num_bsec_outputs; index++)
        {
            switch (bsec_outputs[index].sensor_id)
            {
                case BSEC_OUTPUT_IAQ:
                	bsec_value.iaq = bsec_outputs[index].signal;
                	bsec_value.iaq_accuracy = bsec_outputs[index].accuracy;
                    break;
                case BSEC_OUTPUT_STATIC_IAQ:
                	bsec_value.static_iaq = bsec_outputs[index].signal;
                	bsec_value.static_iaq_accuracy = bsec_outputs[index].accuracy;
                    break;
                case BSEC_OUTPUT_CO2_EQUIVALENT:
                	bsec_value.co2_equivalent = bsec_outputs[index].signal;
                	bsec_value.co2_accuracy = bsec_outputs[index].accuracy;
                    break;
                case BSEC_OUTPUT_BREATH_VOC_EQUIVALENT:
                	bsec_value.breath_voc_equivalent = bsec_outputs[index].signal;
                	bsec_value.breath_voc_accuracy = bsec_outputs[index].accuracy;
                    break;
                case BSEC_OUTPUT_SENSOR_HEAT_COMPENSATED_TEMPERATURE:
                	bsec_value.temp = bsec_outputs[index].signal;
                    break;
                case BSEC_OUTPUT_RAW_PRESSURE:
                	bsec_value.raw_pressure = bsec_outputs[index].signal;
                    break;
                case BSEC_OUTPUT_SENSOR_HEAT_COMPENSATED_HUMIDITY:
                	bsec_value.humidity = bsec_outputs[index].signal;
                    break;
                case BSEC_OUTPUT_RAW_GAS:
                	bsec_value.raw_gas = bsec_outputs[index].signal;
                    break;
                case BSEC_OUTPUT_RAW_TEMPERATURE:
                	bsec_value.raw_temp = bsec_outputs[index].signal;
                    break;
                case BSEC_OUTPUT_RAW_HUMIDITY:
                	bsec_value.raw_humidity = bsec_outputs[index].signal;
                    break;
                case BSEC_OUTPUT_COMPENSATED_GAS:
                	bsec_value.comp_gas_value = bsec_outputs[index].signal;
                	bsec_value.comp_gas_accuracy = bsec_outputs[index].accuracy;
                    break;
                case BSEC_OUTPUT_GAS_PERCENTAGE:
                	bsec_value.gas_percentage = bsec_outputs[index].signal;
                	bsec_value.gas_percentage_acccuracy = bsec_outputs[index].accuracy;
                    break;
                default:
                    continue;
            }
            /* Assume that all the returned timestamps are the same */
            bsec_value.timestamp = bsec_outputs[index].time_stamp;
        }
        /* Pass the extracted outputs to the user provided output_ready() function. */
        output_ready(bsec_value.timestamp, bsec_value.iaq, bsec_value.iaq_accuracy, bsec_value.temp, bsec_value.humidity, bsec_value.raw_pressure,
        		bsec_value.raw_temp, bsec_value.raw_humidity, bsec_value.raw_gas, bsec_status, bsec_value.static_iaq, bsec_value.co2_equivalent, bsec_value.breath_voc_equivalent);
        /* Increment sample counter */
        n_samples++;
        /* Inform master that new measurement data is available after measurement request */

        if (DelayULPPStart == 1) // to synchronize the next ULP_PLUS request with a running ULP cycle
        {
      	  DelayULPPStart = 0;
      	  mMeasurementReq = 1;
        }
