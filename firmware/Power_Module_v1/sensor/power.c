...
//    veml7700_getALS(&power.meas.light.data, &veml7700_config);
    ...
    uint16_t ldrvoltage = ADCC_GetSingleConversion(LDR_VOLT);

    uint16_t tempValue = ADCC_GetSingleConversion(LDR_VOLT);
...
//    ADREFbits.PREF1 = 1;
    ...
    float floatldrvoltage = ((float)ldrvoltage /4096) * 3.3;    //  Convert ADC calue to voltage

    //  equation calculated from actual measurements with LUX meter - see also excel table in iWAST folder
    float LDR = ( ( 4700 * 3.3 ) / floatldrvoltage ) - 4700;    //  Calculate LDR resistance value (ohm) with R = 4.7k and gamma = 0.7
    float lux = 50000000 * pow( (double) LDR, -1.423 );
    
    power.meas.light.data.lux = lux;
